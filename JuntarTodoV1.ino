...
\
  ...
  if (aaaa != "") {
    ...
    pamandarhome += "<form action='/game' method='post'> <br>\
              ...
              <label><input type='radio' name='opcion' value='1'> Ángulo estático</label><br>\
...
void sensor() {
  ...
  if (opcionmando == 1) { //si quisiera hacerlo cambiar la direccion manteniendo un angulo:
    ...
    if (ax > 12000) {
      Direccion = 3; //dirpalabra="izquierda";
    }
    if (ax < -12000) {
      Direccion = 4; //dirpalabra="derecha";
    }
    if (ay > 12000) {
      Direccion = 2; //dirpalabra="abajo";
    }
    if (ay < -12000) {
      Direccion = 1; //dirpalabra="arriba";
    }
  }
